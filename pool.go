...
func (p *pool) Get() (Conn, error) {
	...
	if current == int32(p.opt.MaxActive) {
		...
		//x新版本
		for _, _address := range p.address {
			c, err := p.opt.Dial(_address)
			return p.wrapConn(c, true), err
		}
	}
...
func (p *pool) Status() string {
	return fmt.Sprintf("address:%s, index:%d, current:%d, ref:%d. option:%v", p.address, p.index, p.current, p.ref, p.opt)
}
